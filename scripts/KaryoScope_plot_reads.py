...
def main():
    ...
    if args.batch_size:
        ...
        print(f"\nGenerating {num_batches} batched SVGs ({args.batch_size} reads each)")
        ...
        for i in range(0, len(sorted_reads), args.batch_size):
            ...
            batch_path = f"{base}.batch{batch_num:03d}.{ext}"
            image_height = draw_reads_svg(batch, colors, batch_path, config)
    else:
