...
def render_heatmap(
    ...
    *,
    ...
    absent from, log2fc = -inf, shows at the floor). The format follows the output extension.
    """
    import matplotlib
    ...
    fig_h = max(2.0, 0.26 * len(rows) + 1.2)
    fig_w = max(4.0, 1.1 * len(groups) + 4.0)
    ...
    ax.set_yticklabels(row_labels, fontsize=7, color=fg)
    ax.set_title("Cluster enrichment by sample (log2 fold vs pool)", color=fg)

    # annotate each cell with its (unclamped) value
